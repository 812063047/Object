...
<script type="text/javascript">
	...
	function doEdit(){
		...
		if(rows.length==0){
			$.messager.alert("提示信息","请选择需要修改的取派员","warning");
		}else if(rows.length==1){
			//选中取派员，弹出确认框
			$.messager.confirm("修改确认","你确认要修改选中的取派员吗？",function(r){
				if(r){
					//console.log(rows[0]);
					$('#editStaffWindow').window("open");
					$("#editStaffForm").form("load",rows[0]);
					/* var array=new Array();
					//确定,发送请求
					//获取所有选中取派员的id
			...
			$.messager.alert("提示信息","不能同时选择修改两个以上取派员","warning");
		}
	}
	function doRestore(){
		var rows=$("#grid").datagrid("getSelections");
		if(rows.length==0){
			...
			$.messager.confirm("删除确认","你确认要还原选中的取派员吗？",function(r){
			      if(r){
			    	  var array=new Array();
			    	  for(var i=0;i<rows.length;i++){
							var staff=rows[i]
			    	  ...
			    	  var ids1=array.join(",");
			    	  location.href = "staffAction_restoreBatch.action?ids1="+ids1;
			      }
			});
		}
	}
	...
	function doExport(){
		window.location.href="staffAction_exportXls.action"
	}
	...
	$(function(){
		...
		/*$("#button-import").upload({
			action:'staffAction_importXls.action',
			name:'staffFile'
		});*/
						...
						<script type="text/javascript">
						     $(function(){
						    	 ...
						    	 $("#save").click(function(){
						    		 ...
						    		 if(v){
						    			 $("#addStaffForm").submit();
						    			 $.messager.alert("提示信息","添加取派员成功！","ok");
						    		 }
						    	 });
						    	 //扩展手机号校验规则
						     ...
						     $(function(){
						    	 ...
						    	 $("#edit").click(function(){
						    		 ...
						    		 if(v){
						    			 ...
						    			 $.messager.alert("提示信息","修改取派员成功！","ok");
						    		 }
						    	 })
						    	 //扩展手机号校验规则
						    	 var reg=/^1[3|4|5|7|9][0-9]{9}$/;
	...
	<div class="easyui-window" title="对收派员进行查询" id="searchStaffWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">		
		...
		<div region="north" style="height:31px;overflow:hidden;" split="false" border="false" >
			...
			<form id="searchStaffForm">
				<table class="table-edit" width="80%" align="center" style="height: auto">
					<tr class="title">
					...
					<tr>
							 ...
							 //为保存按钮绑定事件
						    	 $("#search").click(function(){
						    		 ...
						    		 if(v){
						    			 ...
						    			 //$.messager.alert("提示信息","查询取派员成功！","ok");
						    		 }
						    	 })
						    	 //扩展手机号校验规则
						    	 var reg=/^1[3|4|5|7|9][0-9]{9}$/;
						...
						<input type="text" data-options="validType:'telephone'" name="telephone" class="easyui-validatebox" /></td>
					</tr>
					<tr>
						<td>单位</td>
